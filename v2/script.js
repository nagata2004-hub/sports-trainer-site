...
(function() {
  ...
  (function() {
    ...
    const interactiveSelectors = 'a, button, .card, .memory-card, .highlight-card, .news-card, .about__tag';
    document.addEventListener('mouseover', e => {
      if (e.target.closest(interactiveSelectors)) glow.classList.add('hover');
    });
    document.addEventListener('mouseout', e => {
      if (e.target.closest(interactiveSelectors)) glow.classList.remove('hover');
    });
    if (hasGsap) {
      gsap.ticker.add(() => {
      ...
      (function move() { glow.style.left = mx + 'px'; glow.style.top = my + 'px'; requestAnimationFrame(move); })();
    }
  })();
  ...
  if (hasGsap) {
    gsap.utils.toArray('.section-title').forEach(title => {
      ...
      title.innerHTML = chars.map(c =>
        c === '\n' || c === ' ' ? c : `<span class="split-char" style="display:inline-block;opacity:0">${c}</span>`
      ).join('');
      const spans = title.querySelectorAll('.split-char');
      gsap.fromTo(spans,
  ...
  // ===== 4. 画像ズームホバー（CSS側で強化済み） =====

  // ===== 5. セクション間の背景色遷移 =====
  if (hasGsap) {
    const colorSections = [
      { el: '.news-section', bg: '#f2f2f2' },
      { el: '.content-section', bg: '#f8f8f8' },
      { el: '.about', bg: '#f5f0ec' },
      { el: '.board-section', bg: '#f8f8f8' },
      { el: '.contact', bg: '#f0f0f5' },
    ];
    colorSections.forEach(s => {
      const el = document.querySelector(s.el);
      if (!el) return;
      el.classList.add('section-bg-transition');
      ScrollTrigger.create({
        trigger: el,
        start: 'top 70%',
        end: 'bottom 30%',
        onEnter: () => el.style.backgroundColor = s.bg,
        onLeaveBack: () => el.style.backgroundColor = '',
      });
    });
  }
      ...
      for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (dist < 120) {
            ...
            ctx.strokeStyle = `rgba(230, 0, 18, ${0.04 * (1 - dist / 120)})`;
            ctx.stroke();
          }
        }
      }
